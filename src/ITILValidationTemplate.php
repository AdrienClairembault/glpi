...
class ITILValidationTemplate extends AbstractITILChildTemplate
{
    ...
    public static function displayValidatorField(
        ...
        if ($id > 0) {
            ...
            $options['users_id_validate']  = $itemtype == 'user' ? $items_id[0] : $items_id;
            if (isset($target) && isset($target['groups_id'])) {
                $options['users_id_validate']['groups_id'] = $target['groups_id'];
            }
        }
    ...
    {
        if (isset($this->input['validatortype']) && isset($this->input['users_id_validate'])) {
            if (!is_array($this->input['users_id_validate'])) {
                $this->input['users_id_validate'] = [$this->input['users_id_validate']];
            }
            ...
            foreach ($this->input['users_id_validate'] as $user_id) {
                $itilValidationTemplatesTarget->add([
                    'itilvalidationtemplates_id' => $this->getID(),
                    'itemtype'                   => $this->input['validatortype'],
                    'items_id'                   => $user_id,
